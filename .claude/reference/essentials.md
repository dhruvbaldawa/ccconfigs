...
**ultracode**: The slice execution loop as a slim contract (no bundled orchestration script — workflows stay dynamically authored per task). Per slice: sonnet subagent implements, senior-engineer-reviewer + test-reviewer review the uncommitted diff in parallel, sonnet fixes until both approve (3 rejected rounds → block and move on), then atomic commits land per the commit plan. Shared memory in `.ultracode/<task-slug>/context.md` (subagents read first, append after; doubles as resume). Parallel slices only when clearly independent, capped at 2-3, each in its own `git worktree` with in-order merge-back (conflict or red post-merge suite = rejection); external repos always via `git worktree`. Includes the subagent standards block (verify with real output, evidence-backed reporting, scope discipline) injected into every subagent prompt. Loop mechanics are tuned per task and declared in context.md (review cadence, reviewer set, verification depth, rounds cap); tuning never lowers the explicit testing floor (full suite at implement, after every fix, and green before commit). Approvals carry across fix rounds — only the rejecting reviewer re-reviews, unless a fix went beyond their findings, which stales carried approvals and brings both back. Verification evidence is recorded in context.md so later agents build on prior runs — reviewers may always rerun anything. Runs pair with /goal so the stop condition is evaluator-enforced.
