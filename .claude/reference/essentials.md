...
**conveyor**: The slice execution loop as a slim contract (no bundled orchestration script — workflows stay dynamically authored per task; the skill explicitly authorizes the Workflow tool). Per slice: sonnet subagent implements, senior-engineer-reviewer + test-reviewer review the uncommitted diff in parallel (first review full-depth against the whole repo; re-reviews delta-only), sonnet fixes until both approve (at the agreed rounds cap — set per task, no assumed default — or when findings stop shrinking, block and move on), then the orchestrator reruns the declared checks itself and lands atomic commits per the commit plan. Shared memory in `.conveyor/<task-slug>/context.md` (subagents read first, append after; doubles as resume), seeded with the task's **declared checks** — the executable commands that define green for this repo. Parallel slices only when clearly independent, capped at 2-3, each in its own `git worktree` with in-order merge-back (conflict or red post-merge checks = rejection); external repos always via `git worktree`. Includes the subagent standards block (verify with real output, evidence-backed reporting, scope discipline) injected into every subagent prompt. Loop mechanics tuned per task and declared in context.md (review cadence, reviewer set, verification depth, rounds cap); tuning never lowers the testing floor (declared checks at implement, after every fix, green before commit). Approvals carry across fix rounds — only the rejecting reviewer re-reviews, unless a fix went beyond their findings, which stales carried approvals and brings both back at full depth. Reservations/non-blocking findings persist in context.md. Runs pair with /goal so the stop condition is evaluator-enforced.

## Agents

**senior-engineer-reviewer** (claude-opus-4-6[1m], xhigh): Brutal architecture/maintainability review with a round-aware contract — first review judges the diff against the whole repo; re-reviews are delta-only against prior findings. Verdicts: REJECT / NEEDS MAJOR WORK / NEEDS MINOR FIXES / APPROVED WITH RESERVATIONS / SHIP IT — approved means the last two. Writes only to the task workspace, never the code under review. Half of the conveyor review gate.

**test-reviewer** (sonnet, xhigh): Brutal test-quality review — useless tests, flaky patterns, missing assertions, isolation failures; same round-aware contract. Verdicts: GARBAGE / NEEDS WORK / ACCEPTABLE / ACTUALLY GOOD — approved means the last two. The other half of the gate.
